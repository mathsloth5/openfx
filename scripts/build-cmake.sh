...
source ofx-env/Scripts/activate
...
conan install ${GENERATOR_OPTION} -s build_type=$BUILDTYPE --profile:host=C:\\Users\\Opus\\.conan2\\profiles\\default --profile:build=C:\\Users\\Opus\\.conan2\\profiles\\default --build=missing . $conan_opts

echo === Running cmake
# Generate the build files
[[ $USE_OPENCL ]] && cmake_opts="$cmake_opts -DOFX_SUPPORTS_OPENCLRENDER=TRUE"
cmake --preset "conan-release" -DBUILD_EXAMPLE_PLUGINS=TRUE $cmake_opts $ARGS

echo === Building and installing plugins and support libs
echo ${CMAKE_BUILD_DIR}
cmake --build build/Release --target install --config $BUILDTYPE --parallel $VERBOSE
